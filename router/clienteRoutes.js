...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const lancamentos = await Cliente.find();
...
router.get('/relatorio/pdf', async (req, res) => {
  ...
  try {
    ...
    const lancamentos = await Cliente.find(query).sort({ nome: 1, vencimento: 1 });
    ...
    // --- Configurações de Posição e Dimensão ---
    ...
    // Calcular as posições X para cada coluna uma vez
    ...
    // --- Logo, Título e Data de Geração na mesma linha ---
    ...
    // --- Desenha Cabeçalhos da Tabela (CORRIGIDO O ALINHAMENTO VERTICAL) ---

    doc.fontSize(10).font('Helvetica-Bold');

    const tableHeadersLineY = doc.y; // CAPTURA O Y UMA ÚNICA VEZ PARA TODOS OS CABEÇALHOS
    ...
    doc.strokeColor('#aaaaaa')
       ...
       .moveTo(pageMargin, doc.y + rowHeight - 5) // doc.y já avançou após o último cabeçalho
    ...
    let currentY = doc.y; // currentY é atualizado após a linha divisória
    ...
    lancamentos.forEach(lancamento => {
      ...
      if (currentY + rowHeight > doc.page.height - pageMargin) {
        ...
        // Recria Cabeçalhos da Tabela na nova página (CORRIGIDO)

        doc.fontSize(10).font('Helvetica-Bold');

        const newPageTableHeadersLineY = doc.y; // CAPTURA O Y NOVAMENTE PARA A NOVA PÁGINA
